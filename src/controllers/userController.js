...
exports.getUsers = async (req, res, next) => {
    try {
        const { role, status, page = 1, limit = 20, search, branch } = req.query;
        const filter = {};
...
exports.createUser = async (req, res, next) => {
    try {
        ...
        const user = await User.create({ name, email, phone, password, role, branch: branch || null });
...
exports.updateUser = async (req, res, next) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { name, email, phone, role, branch: branch || null }, { new: true, runValidators: true }).populate('branch', 'name code');
